...
namespace AES_forms
{
    class AES
    {
        private static byte[] expandedKey = new byte[176];
        private static int numberOfRounds = 9;
        ...
        static void KeyExpansionCore(ref byte[] inp, byte i)
        {
            //rotate left
            byte t = inp[0];
            inp[0] = inp[1];
        ...
        static void KeyExpansion(byte[] inputKey, ref byte[] expandedKeys)
        {
            //first 16 bytes
            for (int i = 0; i < 16; i++)
            ...
            while (bytesGenerated < 176)
            {
                ...
                {
                    KeyExpansionCore(ref temp, rconIteration);
                    rconIteration++;
                }
                ...
                /*foreach (var item in expandedKeys)
        ...
        public AES(byte[] tkey)
        {
            key = tkey;
            KeyExpansion(key, ref expandedKey);
        }
        ...
        public static void InvMixColumns(ref byte[] state)
        {
            ...
            {
                tmp[i] = (byte)(mul14[state[i]] ^ mul9[state[i + 3]] ^ mul13[state[i + 2]] ^ mul11[state[i + 1]]);
                tmp[i + 1] = (byte)(mul14[state[i + 1]] ^ mul9[state[i]] ^ mul13[state[i + 3]] ^ mul11[state[i + 2]]);
                tmp[i + 2] = (byte)(mul14[state[i + 2]] ^ mul9[state[i + 1]] ^ mul13[state[i]] ^ mul11[state[i + 3]]);
                tmp[i + 3] = (byte)(mul14[state[i + 3]] ^ mul9[state[i + 2]] ^ mul13[state[i + 1]] ^ mul11[state[i]]);
            }
        ...
        public byte[] AESencrypt(byte[] message)
        {
            ...
            for (int i = 0; i < numberOfRounds; i++)
            {
                ...
                Array.Copy(expandedKey, (i + 1) * 16, tempKey, 0, 16);
                AddRoundKey(ref state, tempKey);
            }
            ...
            Array.Copy(expandedKey, 160, finalKey, 0, 16);
            AddRoundKey(ref state, finalKey);

            return state;
        }
        public byte[] AESdecrypt(byte[] cipher)
        {
            byte[] state = new byte[16];
            for (int i = 0; i < 16; i++)
                state[i] = cipher[i];
            ...
            //first round
            Array.Copy(expandedKey, 160, currentKey, 0, 16);
            AddRoundKey(ref state, currentKey);
            InvShiftRows(ref state);
            InvSubBytes(ref state);

            //9-round loop

            for (int i = 0, j = 144; i < numberOfRounds; i++, j -= 16)
            {
                Array.Copy(expandedKey, j, currentKey, 0, 16);
                AddRoundKey(ref state, currentKey);
            ...
            //reversed initial procedure
            Array.Copy(expandedKey, 0, currentKey, 0, 16);
            AddRoundKey(ref state, currentKey);
